...
export default function QrScanScreen({ navigation }) {
  ...
  // Existence check before navigating: a tag printed for a product/location
  // that was since deleted (or an id that never existed) must show a clear
  // message instead of silently landing on an empty screen.
  const verifyExists = async(parsed) => {
    if (parsed.kind === 'product') {
      await apiGet(`/api/products/${parsed.id}`);
    } else if (parsed.kind === 'location') {
      const locations = await apiGet('/api/locations');
      if (!Array.isArray(locations) || !locations.some((l) => Number(l.id) === Number(parsed.id))) {
  ...
  const handleScan = ({ data }) => {
    ...
    // Recognized tag: verify the target still exists server-side, then
    // navigate. The lock is held for the whole await so one tag cannot fire
    // both a 404 alert AND a navigation while the camera keeps streaming.
    lockRef.current = true;
    setBusy(true);
    verifyExists(parsed)
      .then(() => {
      ...
      .catch(() => {
        setLast({ type: 'unknown', raw: `${parsed.kind} #${parsed.id} (no longer exists)` });
        Alert.alert(
          'Tag target not found',
          `This tag points to ${parsed.kind} #${parsed.id}, which no longer exists in the system. The tag may be outdated — reprint it from the admin.`,
        );
      })
      .finally(() => {
        setBusy(false);
        // Re-arm after the cooldown so the operator can scan the next tag.
        setTimeout(() => { lockRef.current = false; }, RESCAN_COOLDOWN_MS);
      });
  };

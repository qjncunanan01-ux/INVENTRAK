...
test('migrateSqliteUsers re-hashes legacy plaintext and leaves hashes alone', () => {
  ...
  assert.strictEqual(
    modern.password,
    before.find(u => u.username === 'modern').password,
    'already-hashed rows untouched'
  );
...
test('npm-free server upgrades a legacy plaintext user on login (Firestore mode)', async () => {
  ...
  store._setDb(fake);
  await store.init();
  store.write('@users', [{ id: 1, username: 'cloud-legacy', password: 'oldpw', role: 'customer', email: 'x@x.com' }]);
  await store.flush();

...
app.put('/api/order-inquiries/:id', authenticateToken, (req, res) => {
  ...
  res.json({ ok: true, message: `Inquiry ${updatedI need to rewrite the full `app.js` with the fix for `const app = express();` and other backward compatibility issues. Let me create the complete corrected file:

<create_file>
<path>c:/Users/Jico/Desktop/INVENTRAK/backend/src/app.js</path>
...
function seedDatabase() {
  ...
  db.transaction(() => {
    ...
    for (const p of products) {
      const result = insertProduct.run(
        p['Product Name'] || p.name, p['Category'] || p.category, p['Brand'] || p.brand || '',
        p['Description'] || '', p['Size'] || p.size || '', p['Unit'] || p.unit || 'pcs',
        p['Price'] || p.price || 0, 'active'
      );
      const pid = result.lastInsertRowid;
      for (const name of locations) {
        const locId = getLocation.get(name).id;
...
app.get('/api/products', (req, res) => {
  ...
  const countRow = db.prepare(`SELECT COUNT(*) as total FROM products ${where}`).get(...params);
  const rows = db.prepare(`SELECT * FROM products ${where} ORDER BY name ASC LIMIT ? OFFSET ?`).all(...params, limitNum, offset);
  // Backward compatibility: if no pagination params, return flat array
...
app.post('/api/products', authenticateToken, adminOnly, validate({
  name: { required: true, maxLength: 200 }, category: { required: true, maxLength: 100 }, price: { required: true, type: 'number', min: 0 }
}), (req, res) => {
  const { name, category, brand, description, size, unit, price, status } = req.body;
  const info = db.prepare('INSERT INTO products (name, category, brand, description, size, unit, price, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .run(name, category, brand || '', description || '', size || '', unit || 'pcs', price, status || 'active');
  res.status(201).json({ id: info.lastInsertRowid });
});
...
app.get('/api/inventory', (req, res) => {
  ...
  if (low_stock === 'true') items = items.filter(item => item.total < 80);
  res.json({ locations, items });
});
...
function consumeStockLots(productId, locationId, quantity) {
  ...
  if (remaining > 0) db.prepare('UPDATE stock SET quantity = quantity - ? WHERE product_id = ? AND location_id = ?').run(remaining, productId, locationId);
}

app.post('/api/stock-movement', authenticateToken, validate({
  product_id: { required: true, type: 'number', min: 1 }, qty: { required: true, type: 'number', min: 0.01 }, type: { required: true, maxLength: 20 }
}), (req, res) => {
  const { product_id, qty, type, src_location, dst_location, notes, user } = req.body;
  const validTypes = ['stock-in', 'stock-out', 'transfer', 'adjustment'];
  ...
  if (type === 'stock-in' && dstId) {
    ...
    const currentStock = db.prepare('SELECT quantity FROM stock WHERE product_id = ? AND location_id = ?').get(product_id, srcId);
    if (!currentStock || currentStock.quantity < qty) return res.status(400).json({ error: 'Insufficient stock at source location' });
    consumeStockLots(product_id, srcId, qty);
    db.prepare('UPDATE stock SET quantity = quantity - ? WHERE product_id = ? AND location_id = ?').run(qty, product_id, srcId);
  } else if (type === 'transfer' && srcId && dstId) {
    const currentStock = db.prepare('SELECT quantity FROM stock WHERE product_id = ? AND location_id = ?').get(product_id, srcId);
...
app.get('/api/stock-movements', (req, res) => {
  ...
  const countRow = db.prepare(`SELECT COUNT(*) as total FROM stock_movements ${where}`).get(...params);
  const rows = db.prepare(`SELECT * FROM stock_movements ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(...params, limitNum, offset);
  // Backward compatibility: return flat array if no pagination
...
app.post('/api/locations', authenticateToken, adminOnly, validate({ name: { required: true, maxLength: 100 } }), (req, res) => {
  const { name } = req.body;
  const existing = db.prepare('SELECT id FROM locations WHERE name = ?').get(name);
  if (existing) return res.status(409).json({ error: 'Location already exists' });
  const info = db.prepare('INSERT INTO locations (name) VALUES (?)').run(name);
...
app.get('/api/optimization/:productId', (req, res) => {
  ...
  const avgInventory = currentStock?.total || 1;
  const turnover = annualDemand / avgInventory;
  res.json({ EOQ: Math.round(EOQ), ROP, safetyStock, annualDemand, turnoverRatio: Math.round(turnover * 100) / 100, avgInventory: Math.round(avgInventory) });
});
...
app.get('/api/optimization', (req, res) => {
  ...
  const results = products.map(p => {
    ...
    const currentStock = db.prepare('SELECT SUM(quantity) as total FROM stock WHERE product_id = ?').get(p.id);
    const avgInv = currentStock?.total || 1;
    return { productId: p.id, productName: p.name, eoq: Math.round(EOQ), annualDemand, turnoverRatio: Math.round((annualDemand / avgInv) * 100) / 100 };
  });
  res.json(results);
});
...
app.post('/api/order-inquiries', validate({ customer_name: { required: true, maxLength: 100 }, customer_email: { required: true, maxLength: 100 } }), (req, res) => {
...
app.get('/api/analytics/summary', (req, res) => {
  ...
  res.json({ totalProducts, totalStock, lowStockItems, totalLocations, pendingInquiries, totalSales, totalMovements, activeAlerts, topProducts, monthlyMovements });
});
...
app.get('/api/analytics/export/:type', authenticateToken, adminOnly, (req, res) => {
  ...
  let data;
  switch (type) {
    case 'products': data = db.prepare('SELECT * FROM products WHERE status = ?').all('active'); break;
    case 'inventory': data = db.prepare('SELECT p.name as product, l.name as location, s.quantity FROM stock s JOIN products p ON s.product_id = p.id JOIN locations l ON s.location_id = l.id WHERE p.status = ? ORDER BY p.name, l.name').all('active'); break;
    case 'movements': data = db.prepare('SELECT * FROM stock_movements ORDER BY created_at DESC LIMIT 1000').all(); break;
    default: return res.status(404).json({ error: 'Export type not found. Use: products, inventory, movements' });
  }
  if (format === 'csv') {
...
app.post('/api/sales', authenticateToken, validate({ product_id: { required: true, type: 'number', min: 1 }, qty: { required: true, type: 'number', min: 0.01 } }), (req, res) => {
  ...
  db.prepare('INSERT INTO sales_transactions (product_id, qty, unit_price, total_amount, customer_name) VALUES (?, ?, ?, ?, ?)').run(product_id, qty, product.price, total, customer_name || req.user?.username || 'anonymous');
